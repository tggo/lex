...
func testClient(t *testing.T, srv *httptest.Server) *client {
	return &client{
		cfg:     Config{ListBase: srv.URL, EISBBase: srv.URL, UA: "x", Client: srv.Client()},
...
func TestFetch_retryThenSuccess(t *testing.T) {
	...
	b, err := testClient(t, srv).fetch(context.Background(), srv.URL, "text/html")
	if err != nil {
...
func TestFetch_nonRetryableStatus(t *testing.T) {
	...
	defer srv.Close()
	if _, err := testClient(t, srv).fetch(context.Background(), srv.URL, "text/html"); err == nil {
...
func TestFetch_contextCancelDuringBackoff(t *testing.T) {
	...
	if _, err := testClient(t, srv).fetch(ctx, srv.URL, "text/html"); err == nil {
