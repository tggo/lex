...
// TestMCPQueries_golden is the full stack as a test: import the RIS fixtures into
...
func TestMCPQueries_golden(t *testing.T) {
	...
	// The sole fixture act: Verordnung über Pflichtnotstandsreserven an Erdöl.
	act := schema.ResourceURI("at", "verordnung", 1990, sampleGN)
	...
	r1, e1 := svc.SearchLaws(ctx, &mcp.SearchIn{Query: "Pflichtnotstandsreserven"})
	q("search_pflichtnotstandsreserven", r1, e1)
	// Full-text search: a word that lives in the ARTICLE TEXT.
	r2, e2 := svc.SearchLaws(ctx, &mcp.SearchIn{Query: "Erdöl", Limit: 5})
	q("search_erdoel", r2, e2)
	...
	// Amend/repeal edges parsed from the head document.
