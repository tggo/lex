...
// TestMCPQueries_golden is the full stack as a test: import the Finlex fixtures
...
func TestMCPQueries_golden(t *testing.T) {
	...
	act := schema.ResourceURI("fi", "laki", 2019, "2019/469")
	...
	// A word from the section headings/body of act 2019/469 (etuosto-oikeus).
	r1, e1 := svc.SearchLaws(ctx, &mcp.SearchIn{Query: "etuosto"})
	q("search_etuosto", r1, e1)
	// Another body word (valtion = the state's pre-emption right).
	r2, e2 := svc.SearchLaws(ctx, &mcp.SearchIn{Query: "valtion", Limit: 5})
	q("search_valtion", r2, e2)
	// Retrieve the act with its as-of date and articles.
	r3, e3 := svc.GetAct(ctx, &mcp.ActIn{URI: act})
	q("get_act", r3, e3)
	// A single section/article.
	r4, e4 := svc.GetArticle(ctx, &mcp.ArticleIn{ActURI: act, Number: "1"})
	q("get_article_1", r4, e4)
	// Amend edges from the AKN fixture (amendedBy 2022/1099).
