...
func TestFetch_retryThenSuccess(t *testing.T) {
	...
	b, err := testClient(srv).fetch(context.Background(), srv.URL)
	if err != nil {
...
func TestFetch_429Retryable(t *testing.T) {
	...
	defer srv.Close()
	if _, err := testClient(srv).fetch(context.Background(), srv.URL); err != nil {
		t.Fatalf("fetch: %v", err)
	}
}
...
func TestFetch_contextCancelDuringBackoff(t *testing.T) {
	...
	if _, err := testClient(srv).fetch(ctx, srv.URL); err == nil {
