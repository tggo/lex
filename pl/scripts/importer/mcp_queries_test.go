...
// TestMCPQueries_golden is the full stack as a test: import the eli fixtures
...
func TestMCPQueries_golden(t *testing.T) {
	...
	act := schema.ResourceURI("pl", "ustawa", 2023, "DU/2777")
	...
	// Title word.
	r1, e1 := svc.SearchLaws(ctx, &mcp.SearchIn{Query: "budżetowej", Limit: 5})
	q("search_budzetowej", r1, e1)
	// Article-text word.
	r2, e2 := svc.SearchLaws(ctx, &mcp.SearchIn{Query: "wynagrodzenie", Limit: 5})
	q("search_wynagrodzenie", r2, e2)
	...
	// Amend edges resolved from references fixture.
